...
Audience: Operators setting up and running agents in GitMesh Agents

## 1. What this system does

Agents in GitMesh Agents do not run continuously.  
...
For `claude_local` and `codex_local`, GitMesh Agents assumes the CLI is already installed and authenticated on the host machine.
...
GitMesh Agents stores session IDs for resumable adapters.
...
GitMesh Agents pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. GitMesh Agents surfaces this as a warning in environment tests, not a hard error.
