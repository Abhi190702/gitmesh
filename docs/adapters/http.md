...
The `http` adapter sends a webhook request to an external agent service. The agent runs externally and GitMesh Agents just triggers it.
...
1. GitMesh Agents sends a POST request to the configured URL
2. The request body includes the execution context (agent ID, task info, wake reason)
3. The external agent processes the request and calls back to the GitMesh Agents API
...
The external agent uses `GITMESH_API_URL` and an API key to call back to GitMesh Agents.
