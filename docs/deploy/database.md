...
GitMesh Agents uses PostgreSQL via Drizzle ORM. There are three ways to run the database.
...
1. Creates `~/.gitmesh-agents/instances/default/db/` for storage
2. Ensures the `gitmesh-agents` database exists
3. Runs migrations automatically
4. Starts serving requests

Data persists across restarts. To reset: `rm -rf ~/.gitmesh-agents/instances/default/db`.
...
```sh
cp .env.example .env
# DATABASE_URL=postgres://gitmesh:gitmesh@localhost:5432/gitmesh-agents
```
...
The Drizzle schema (`lib/data/src/schema/`) is the same regardless of mode.
