...
export const configMetaSchema = z.object({
  version: z.literal(1),
  updatedAt: z.string(),
  source: z.enum(["setup", "configure", "doctor"]),
});
...
export const databaseBackupConfigSchema = z.object({
  ...
  dir: z.string().default("~/.gitmesh-agents/instances/default/data/backups"),
});

export const databaseConfigSchema = z.object({
  mode: z.enum(["embedded-postgres", "postgres"]).default("embedded-postgres"),
  connectionString: z.string().optional(),
  embeddedPostgresDataDir: z.string().default("~/.gitmesh-agents/instances/default/db"),
  embeddedPostgresPort: z.number().int().min(1).max(65535).default(54329),
  backup: databaseBackupConfigSchema.default({
    enabled: true,
    intervalMinutes: 60,
    retentionDays: 30,
    dir: "~/.gitmesh-agents/instances/default/data/backups",
  }),
});

export const loggingConfigSchema = z.object({
  mode: z.enum(["file", "cloud"]),
  logDir: z.string().default("~/.gitmesh-agents/instances/default/logs"),
});
...
export const storageLocalDiskConfigSchema = z.object({
  baseDir: z.string().default("~/.gitmesh-agents/instances/default/data/storage"),
});

export const storageS3ConfigSchema = z.object({
  bucket: z.string().min(1).default("gitmesh-agents"),
...
export const storageConfigSchema = z.object({
  ...
  localDisk: storageLocalDiskConfigSchema.default({
    baseDir: "~/.gitmesh-agents/instances/default/data/storage",
  }),
  s3: storageS3ConfigSchema.default({
    bucket: "gitmesh-agents",
    region: "us-east-1",
    prefix: "",
...
export const secretsLocalEncryptedConfigSchema = z.object({
  keyFilePath: z.string().default("~/.gitmesh-agents/instances/default/secrets/master.key"),
});
...
export const secretsConfigSchema = z.object({
  ...
  localEncrypted: secretsLocalEncryptedConfigSchema.default({
    keyFilePath: "~/.gitmesh-agents/instances/default/secrets/master.key",
  }),
});

export const gitmeshConfigSchema = z
  .object({
...
export type GitmeshConfig = z.infer<typeof gitmeshConfigSchema>;
