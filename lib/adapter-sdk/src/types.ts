...
/** Payload for the onEnableApproved adapter lifecycle hook (e.g. join-request or enable_agent approval). */
export interface EnableApprovedPayload {
  projectId: string;
  agentId: string;
  agentName: string;
  adapterType: string;
...
/** Result of onEnableApproved hook; failures are non-fatal to the approval flow. */
export interface EnableApprovedHookResult {
  ok: boolean;
  error?: string;
...
export interface ServerAdapterModule {
  ...
  /**
   * Optional lifecycle hook when an agent is approved/enabled (join-request or enable_agent approval).
   * adapterConfig is the agent's adapter config so the adapter can e.g. send a callback to a configured URL.
   */
  onEnableApproved?: (
    payload: EnableApprovedPayload,
    adapterConfig: Record<string, unknown>,
  ) => Promise<EnableApprovedHookResult>;
}
