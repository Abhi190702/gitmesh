...
export function buildGitmeshEnv(agent: { id: string; projectId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    GITMESH_AGENT_ID: agent.id,
    GITMESH_PROJECT_ID: agent.projectId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.GITMESH_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.GITMESH_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.GITMESH_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.GITMESH_API_URL = apiUrl;
  return vars;
}
