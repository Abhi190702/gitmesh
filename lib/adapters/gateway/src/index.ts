export const type = "gateway";
export const label = "Gateway";

export const models: { id: string; label: string }[] = [];

export const agentConfigurationDoc = `# gateway agent configuration

Adapter: gateway

Use when:
- You want Gitmesh to invoke Gateway over the Gateway WebSocket protocol.
- You want native gateway auth/connect semantics instead of HTTP /v1/responses or /hooks/*.

Don't use when:
- You only expose Gateway HTTP endpoints.
- Your deployment does not permit outbound WebSocket access from the GitMesh Agents server.

Core fields:
- url (string, required): Gateway gateway WebSocket URL (ws:// or wss://)
- headers (object, optional): handshake headers; supports x-gateway-token / x-gateway-auth
...
- gitmeshApiUrl (string, optional): absolute Gitmesh base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default gitmesh-agents)
`;
