...
import { asString, parseObject } from "@gitmesh/adapter-sdk/server-utils";
...
export async function testEnvironment(
  ...
  if (!urlValue) {
    checks.push({
      ...
      level: "error",
      message: "Gateway gateway adapter requires a WebSocket URL.",
  ...
  if (url && url.protocol !== "ws:" && url.protocol !== "wss:") {
    checks.push({
      code: "gateway_url_protocol_invalid",
      level: "error",
  ...
  if (url) {
    ...
    if (url.protocol === "ws:" && !isLoopbackHost(url.hostname)) {
      checks.push({
        code: "gateway_plaintext_remote_ws",
        level: "warn",
  ...
  if (authToken || password) {
    ...
    checks.push({
      ...
      hint: "Set authToken/password or headers.x-gateway-token for authenticated gateways.",
    });
  }
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the GitMesh Agents server host.",
        });
      }
    } catch (err) {
      checks.push({
