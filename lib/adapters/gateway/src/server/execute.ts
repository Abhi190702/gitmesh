...
import { asNumber, asString, buildGitmeshEnv, parseObject } from "@gitmesh/adapter-sdk/server-utils";
...
const DEFAULT_CLIENT_VERSION = "gitmesh-agents";
const DEFAULT_ROLE = "operator";

const SENSITIVE_LOG_KEY_PATTERN =
  /(^|[_-])(auth|authorization|token|secret|password|api[_-]?key|private[_-]?key)([_-]|$)|^x-gateway-(auth|token)$/i;
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "gitmesh-agents";
  if (input.strategy === "run") return `gitmesh-agents:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `gitmesh-agents:issue:${input.issueId}`;
  return fallback;
}
...
function resolveAuthToken(config: Record<string, unknown>, headers: Record<string, string>): string | null {
  ...
  const tokenHeader = headerMapGetIgnoreCase(headers, "x-gateway-token");
...
function resolveGitmeshApiUrlOverride(value: unknown): string | null {
...
function buildGitmeshEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const gitmeshApiUrlOverride = resolveGitmeshApiUrlOverride(ctx.config.gitmeshApiUrl);
  const gitmeshEnv: Record<string, string> = {
    ...buildGitmeshEnv(ctx.agent),
    GITMESH_RUN_ID: ctx.runId,
  };

  if (gitmeshApiUrlOverride) {
    gitmeshEnv.GITMESH_API_URL = gitmeshApiUrlOverride;
  }
  if (wakePayload.taskId) gitmeshEnv.GITMESH_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) gitmeshEnv.GITMESH_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) gitmeshEnv.GITMESH_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) gitmeshEnv.GITMESH_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) gitmeshEnv.GITMESH_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    gitmeshEnv.GITMESH_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return gitmeshEnv;
}

function buildWakeText(payload: WakePayload, gitmeshEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.gitmesh-agents/workspace/gitmesh-agents-claimed-api-key.json";
  const orderedKeys = [
    "GITMESH_RUN_ID",
    "GITMESH_AGENT_ID",
    "GITMESH_PROJECT_ID",
  ...
  for (const key of orderedKeys) {
    const value = gitmeshEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = gitmeshEnv.GITMESH_API_URL ?? "<set GITMESH_API_URL>";

  const lines = [
    "Gitmesh wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `GITMESH_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load GITMESH_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $GITMESH_API_KEY on every API call.",
    "- Use X-Gitmesh-Run-Id: $GITMESH_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: GITMESH_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$GITMESH_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/projects/$GITMESH_PROJECT_ID/issues?assigneeAgentId=$GITMESH_AGENT_ID&status=todo,in_progress,blocked",
    "   - Pick in_progress first, then todo, then blocked, then execute step 3.",
    "",
    "Useful endpoints for issue work:",
    "- POST /api/issues/{issueId}/comments",
    "- PATCH /api/issues/{issueId}",
    "- POST /api/projects/{projectId}/issues (when asked to create a new issue)",
    "",
...
class GatewayWsClient {
  ...
  async connect(
    ...
    ws.on("error", (err) => {
      const message = err instanceof Error ? err.message : String(err);
      void this.opts.onLog("stderr", `[gateway] websocket error: ${message}\n`);
    });
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "gitmesh-agents-complete");
    this.ws = null;
  }
...
async function autoApproveDevicePairing(params: {
  ...
  try {
    await params.onLog(
      "stdout",
      "[gateway] pairing required; attempting automatic pairing approval via gateway methods\n",
    );
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (!urlValue) {
    return {
      ...
      signal: null,
      timedOut: false,
      errorMessage: "Gateway gateway adapter missing url",
      errorCode: "gateway_url_missing",
    };
  }
  ...
  if (parsedUrl.protocol !== "ws:" && parsedUrl.protocol !== "wss:") {
    return {
      ...
      errorCode: "gateway_url_protocol",
    };
  }
  ...
  const gitmeshEnv = buildGitmeshEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, gitmeshEnv);
  ...
  await ctx.onLog(
    "stdout",
    `[gateway] outbound headers (redacted): ${stringifyForLog(redactForLog(headers), 4_000)}\n`,
  );
  await ctx.onLog(
    "stdout",
    `[gateway] outbound payload (redacted): ${stringifyForLog(redactForLog(agentParams), 12_000)}\n`,
  );
  await ctx.onLog("stdout", `[gateway] outbound header keys: ${outboundHeaderKeys.join(", ")}\n`);
  if (transportHint) {
    await ctx.onLog(
      "stdout",
      `[gateway] ignoring streamTransport=${transportHint}; gateway adapter always uses websocket protocol\n`,
    );
  }
  if (parsedUrl.protocol === "ws:" && !isLoopbackHost(parsedUrl.hostname)) {
    await ctx.onLog(
      "stdout",
      "[gateway] warning: using plaintext ws:// to a non-loopback host; prefer wss:// for remote endpoints\n",
    );
  }
  ...
  while (true) {
    ...
    const onEvent = async (frame: GatewayEventFrame) => {
      if (frame.event !== "agent") {
        if (frame.event === "shutdown") {
          await ctx.onLog(
            "stdout",
            `[gateway] gateway shutdown notice: ${stringifyForLog(frame.payload ?? {}, 2_000)}\n`,
          );
        }
        return;
      }
      ...
      await ctx.onLog(
        "stdout",
        `[gateway:event] run=${runId} stream=${stream} data=${stringifyForLog(data, 8_000)}\n`,
      );
    ...
    try {
      ...
      if (deviceIdentity) {
        await ctx.onLog(
          "stdout",
          `[gateway] device auth enabled keySource=${deviceIdentity.source} deviceId=${deviceIdentity.deviceId}\n`,
        );
      } else {
        await ctx.onLog("stdout", "[gateway] device auth disabled\n");
      }

      await ctx.onLog("stdout", `[gateway] connecting to ${parsedUrl.toString()}\n`);
      ...
      await ctx.onLog(
        "stdout",
        `[gateway] connected protocol=${asNumber(asRecord(hello)?.protocol, PROTOCOL_VERSION)}\n`,
      );
      ...
      await ctx.onLog(
        "stdout",
        `[gateway] agent accepted runId=${acceptedRunId} status=${acceptedStatus || "unknown"}\n`,
      );

      if (acceptedStatus === "error") {
        const errorMessage =
          nonEmpty(acceptedPayload?.summary) ?? lifecycleError ?? "Gateway gateway agent request failed";
        return {
          exitCode: 1,
          signal: null,
          timedOut: false,
          errorMessage,
          errorCode: "gateway_agent_error",
      ...
      if (acceptedStatus !== "ok") {
        ...
        if (waitStatus === "timeout") {
          return {
            ...
            signal: null,
            timedOut: true,
            errorMessage: `Gateway gateway run timed out after ${waitTimeoutMs}ms`,
            errorCode: "gateway_wait_timeout",
        ...
        if (waitStatus === "error") {
          return {
            ...
            errorMessage:
              ...
              lifecycleError ??
              "Gateway gateway run failed",
            errorCode: "gateway_wait_error",
            resultJson: waitPayload,
          };
        }
        ...
        if (waitStatus && waitStatus !== "ok") {
          return {
            ...
            signal: null,
            timedOut: false,
            errorMessage: `Unexpected Gateway gateway agent.wait status: ${waitStatus}`,
            errorCode: "gateway_wait_status_unexpected",
            resultJson: waitPayload,
          };
        }
      }
      ...
      const provider = nonEmpty(agentMeta?.provider) ?? nonEmpty(meta?.provider) ?? "gateway";
      const model = nonEmpty(agentMeta?.model) ?? nonEmpty(meta?.model) ?? null;
      const costUsd = asNumber(agentMeta?.costUsd ?? meta?.costUsd, 0);

      await ctx.onLog(
        "stdout",
        `[gateway] run completed runId=${Array.from(trackedRunIds).join(",")} status=ok\n`,
      );
      ...
      if (
        ...
        if (pairResult.ok) {
          await ctx.onLog(
            "stdout",
            `[gateway] auto-approved pairing request ${pairResult.requestId}; retrying\n`,
          );
          continue;
        }
        await ctx.onLog(
          "stderr",
          `[gateway] auto-pairing failed: ${pairResult.reason}\n`,
        );
      }

      const detailedMessage = pairingRequired
        ? `${message}. Approve the pending device via the gateway (for example: gateway devices approve --latest --url <gateway-ws-url> --token <gateway-token>) and retry. Ensure this agent has a persisted adapterConfig.devicePrivateKeyPem so approvals are reused.`
        : message;

      await ctx.onLog("stderr", `[gateway] request failed: ${detailedMessage}\n`);
