...
import { normalizeGatewayStreamLine } from "../shared/stream.js";
...
function parseAgentEventLine(line: string, ts: string): TranscriptEntry[] {
  const match = line.match(/^\[gateway:event\]\s+run=([^\s]+)\s+stream=([^\s]+)\s+data=(.*)$/s);
...
export function parseGatewayStdoutLine(line: string, ts: string): TranscriptEntry[] {
  const normalized = normalizeGatewayStreamLine(line);
  ...
  if (trimmed.startsWith("[gateway:event]")) {
    return parseAgentEventLine(trimmed, ts);
  }

  if (trimmed.startsWith("[gateway]")) {
    return [{ kind: "system", ts, text: trimmed.replace(/^\[gateway\]\s*/, "") }];
  }
