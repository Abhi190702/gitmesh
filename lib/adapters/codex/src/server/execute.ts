...
const GITMESH_PLAYBOOK_CANDIDATES = [
  path.resolve(__moduleDir, "../../playbooks"),         // published: <pkg>/dist/server/ -> <pkg>/playbooks/
  path.resolve(__moduleDir, "../../../../../playbooks"), // dev: src/server/ -> repo root/playbooks/
];
...
async function ensureCodexPlaybooksInjected(onLog: AdapterExecutionContext["onLog"], role?: string) {
  const playbooksDir = await resolveGitmeshPlaybooksDir();
  if (!playbooksDir) return;

  const playbooksHome = path.join(codexHomeDir(), "playbooks");
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[gitmesh-agents] Injected Codex skill "${entry.name}" into ${playbooksHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[gitmesh-agents] Failed to inject Codex skill "${entry.name}" into ${playbooksHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  await ensureCodexPlaybooksInjected(onLog, agent.role ?? undefined);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[gitmesh-agents] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[gitmesh-agents] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
