import { asString, asBoolean, asNumber, asStringArray, parseObject, ensureAbsoluteDirectory, ensureCommandResolvable, ensurePathInEnv, runChildProcess, } from "@gitmesh/adapter-sdk/server-utils";
...
export async function testEnvironment(ctx) {
    ...
    if (canRunProbe) {
        if (!commandLooksLike(command, "claude")) {
            ...
            const probe = await runChildProcess(`claude-gateway-envtest-${Date.now()}-${Math.random().toString(16).slice(2)}`, command, args, {
                cwd,
                env: runtimeEnv,
                timeoutSec: 60,
                graceSec: 5,
