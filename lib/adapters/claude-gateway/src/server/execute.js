...
import { asString, asNumber, asStringArray, parseObject, buildGitmeshEnv, redactEnvForLogs, ensureAbsoluteDirectory, ensureCommandResolvable, ensurePathInEnv, runChildProcess, } from "@gitmesh/adapter-sdk/server-utils";
import { parseClaudeStreamJson, describeClaudeFailure, detectClaudeLoginRequired, } from "../../../claude/src/server/parse.js";
...
async function resolveGitmeshPlaybooksDir() {
...
async function buildPlaybooksDir(role) {
    ...
    for (const entry of entries) {
        if (!entry.isDirectory())
            continue;
        if (allowedPlaybooks && !allowedPlaybooks.has(entry.name))
            continue;
        await fs.symlink(path.join(playbooksDir, entry.name), path.join(target, entry.name));
    }
    return tmp;
}
 ...
 */
function resolveProviderConfig(providerName, customBaseUrl) {
...
async function buildClaudeGatewayRuntimeConfig(input) {
    ...
    const workspaceHints = Array.isArray(context.gitmeshWorkspaces)
        ? context.gitmeshWorkspaces.filter((value) => typeof value === "object" && value !== null)
        : [];
    ...
    const hasExplicitApiKey = typeof envConfig.GITMESH_API_KEY === "string" && envConfig.GITMESH_API_KEY.trim().length > 0;
    const env = { ...buildGitmeshEnv(agent) };
    ...
    if (!apiKey) {
        throw new Error(`[claude_gateway] Missing required configuration: apiKey. Provider: ${providerName}`);
    }
...
export async function runClaudeGatewayLogin(input) {
    ...
    const proc = await runChildProcess(input.runId, runtime.command, ["login", ...runtime.extraArgs], {
        cwd: runtime.cwd,
        env: runtime.env,
...
export async function execute(input) {
    ...
    await onLog_("stdout", `[claude_gateway] Environment: ${Object.keys(redactedEnv)
        .filter((k) => k.startsWith("GITMESH_") || k.startsWith("ANTHROPIC_"))
        .join(", ")}\n`);
    const proc = await runChildProcess(runId, runtimeConfig.command, runtimeConfig.extraArgs, {
    ...
    const failureDescription = describeClaudeFailure(streamObjects[0] ?? {});
