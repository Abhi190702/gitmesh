...
function renderGitmeshEnvNote(env: Record<string, string>): string {
  const gitmeshKeys = Object.keys(env)
    .filter((key) => key.startsWith("GITMESH_"))
    .sort();
  if (gitmeshKeys.length === 0) return "";
  return [
    "GitMesh Agents runtime note:",
    `The following GITMESH_* environment variables are available in this run: ${gitmeshKeys.join(", ")}`,
...
function cursorSkillsHome(): string {
  return path.join(os.homedir(), ".cursor", "playbooks");
}
...
type EnsureCursorSkillsInjectedOptions = {
  playbooksDir?: string | null;
  playbooksHome?: string;
  linkSkill?: (source: string, target: string) => Promise<void>;
  role?: string;
};

export async function ensureCursorPlaybooksInjected(
  onLog: AdapterExecutionContext["onLog"],
  options: EnsureCursorSkillsInjectedOptions = {},
) {
  const playbooksDir = options.playbooksDir ?? await resolveGitmeshPlaybooksDir();
  if (!playbooksDir) return;

  const playbooksHome = options.playbooksHome ?? cursorSkillsHome();
  try {
    await fs.mkdir(playbooksHome, { recursive: true });
  } catch (err) {
    await onLog(
      "stderr",
      `[gitmesh-agents] Failed to prepare Cursor playbooks directory ${playbooksHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[gitmesh-agents] Failed to read Gitmesh playbooks from ${playbooksDir}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[gitmesh-agents] Injected Cursor skill "${entry.name}" into ${playbooksHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[gitmesh-agents] Failed to inject Cursor skill "${entry.name}" into ${playbooksHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  await ensureCursorPlaybooksInjected(onLog, { role: agent.role ?? undefined });
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[gitmesh-agents] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[gitmesh-agents] Loaded agent instructions file: ${instructionsFilePath}\n`,
      );
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      await onLog(
        "stderr",
        `[gitmesh-agents] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
    }
  }
  const commandNotes = (() => {
  ...
  const gitmeshEnvNote = renderGitmeshEnvNote(env);
  const prompt = `${instructionsPrefix}${gitmeshEnvNote}${renderedPrompt}`;
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[gitmesh-agents] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
