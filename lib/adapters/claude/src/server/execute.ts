...
/**
 * Create a tmpdir with `.claude/playbooks/` containing symlinks to playbooks from
 * the repo's `playbooks/` directory, so `--add-dir` makes Claude Code discover
 * them as proper registered playbooks.
 *
 * When a role is provided, only the playbooks relevant to that role are injected
 * (via getPlaybooksForRole). When omitted, all skill directories are injected.
 */
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (instructionsFilePath) {
    ...
    const combinedPath = path.join(playbooksDir, "agent-instructions.md");
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[gitmesh-agents] Claude session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const buildClaudeArgs = (resumeSessionId: string | null) => {
    ...
    args.push("--add-dir", playbooksDir);
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stderr",
        `[gitmesh-agents] Claude resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
    ...
    fs.rm(playbooksDir, { recursive: true, force: true }).catch(() => {});
  }
}
