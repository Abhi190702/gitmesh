...
import type { AdapterModel } from "@gitmesh/adapter-sdk";
import { asString, runChildProcess } from "@gitmesh/adapter-sdk/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.GITMESH_PI_COMMAND === "string" &&
    process.env.GITMESH_PI_COMMAND.trim().length > 0
      ? process.env.GITMESH_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["GITMESH_", "npm_", "NPM_"] as const;
