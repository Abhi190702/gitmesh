...
const GITMESH_SESSIONS_DIR = path.join(os.homedir(), ".pi", "gitmesh-agents");
...
async function ensurePiPlaybooksInjected(onLog: AdapterExecutionContext["onLog"], role?: string) {
  const playbooksDir = await resolveGitmeshPlaybooksDir();
  if (!playbooksDir) return;

  const piSkillsHome = path.join(os.homedir(), ".pi", "agent", "playbooks");
  ...
  for (const entry of entries) {
    ...
    const source = path.join(playbooksDir, entry.name);
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[gitmesh-agents] Injected Pi skill "${entry.name}" into ${piSkillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[gitmesh-agents] Failed to inject Pi skill "${entry.name}" into ${piSkillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(GITMESH_SESSIONS_DIR, { recursive: true });
  return GITMESH_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(GITMESH_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const promptTemplate = asString(
    ...
    "You are agent {{agent.id}} ({{agent.name}}). Continue your Gitmesh work.",
  );
  ...
  // Inject playbooks
  await ensurePiPlaybooksInjected(onLog, agent.role ?? undefined);
  ...
  if (wakeTaskId) env.GITMESH_TASK_ID = wakeTaskId;
  if (wakeReason) env.GITMESH_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.GITMESH_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.GITMESH_APPROVAL_ID = approvalId;
  if (approvalStatus) env.GITMESH_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.GITMESH_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (workspaceCwd) env.GITMESH_WORKSPACE_CWD = workspaceCwd;
  if (workspaceSource) env.GITMESH_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.GITMESH_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.GITMESH_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.GITMESH_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (workspaceHints.length > 0) env.GITMESH_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[gitmesh-agents] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[gitmesh-agents] Loaded agent instructions file: ${resolvedInstructionsFilePath}\n`,
      );
    } catch (err) {
      ...
      await onLog(
        "stderr",
        `[gitmesh-agents] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[gitmesh-agents] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
