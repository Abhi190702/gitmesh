...
Use when:
- You want Gitmesh to run Pi (the AI coding agent) locally as the agent runtime
...
- Gitmesh requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/gitmesh-agents/ and resumed with --session.
