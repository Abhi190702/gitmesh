...
export const agentRuntimeState = pgTable(
  ...
  (table) => ({
    projectAgentIdx: index("agent_runtime_state_project_agent_idx").on(table.projectId, table.agentId),
    projectUpdatedIdx: index("agent_runtime_state_project_updated_idx").on(table.projectId, table.updatedAt),
  }),
);
