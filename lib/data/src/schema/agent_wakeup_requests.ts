...
export const agentWakeupRequests = pgTable(
  "agent_wakeup_requests",
  {
    ...
    runId: uuid("run_id"),
    /** Forge event type that triggered this wakeup (e.g., pr_opened, issue_opened) */
    forgeEventType: text("forge_event_type"),
  ...
  (table) => ({
    projectAgentStatusIdx: index("agent_wakeup_requests_project_agent_status_idx").on(
      table.projectId,
      table.agentId,
      table.status,
    ),
    projectRequestedIdx: index("agent_wakeup_requests_project_requested_idx").on(
      table.projectId,
      table.requestedAt,
    ),
