...
function resolveGitmeshHomeDir(): string {
  const envHome = process.env.GITMESH_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".gitmesh-agents");
}

function resolveGitmeshInstanceId(): string {
  const raw = process.env.GITMESH_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveGitmeshHomeDir(), "instances", resolveGitmeshInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveGitmeshHomeDir(), "instances", resolveGitmeshInstanceId(), "data", "backups");
}
