...
GitMesh Agents supports two runtime modes:
...
## 6. Operator/User Integration Contract

Operator identity must be represented by a real DB user principal for user-based features to work consistently.

Required integration points:

- real user row in `authUsers` for Operator identity
- `instance_user_roles` entry for Operator admin authority
- `project_memberships` integration for user-level task assignment and access
...
When running `authenticated` mode, if the only instance admin is `local-operator`, GitMesh Agents emits a startup warning with a one-time high-entropy claim URL.

- URL format: `/operator-claim/<token>?code=<code>`
- intended use: signed-in human claims operator ownership
- claim action:
  - promotes current signed-in user to `instance_admin`
  - demotes `local-operator` admin role
  - ensures active owner membership for the claiming user across existing projects
...
- `local_trusted` ensures a real local Operator user principal in `authUsers` with `instance_user_roles` admin access
- project creation ensures creator membership in `project_memberships` so user assignment/access flows remain consistent
