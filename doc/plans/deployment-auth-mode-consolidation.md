...
Keep GitMesh Agents low-friction while making the mode model simpler and safer:
...
- Runtime deployment modes are currently `local_trusted | cloud_hosted` (`lib/core/src/constants.ts`).
...
- `cloud_hosted` uses Better Auth sessions and `authUsers` rows (`server/src/auth/better-auth.ts`, `lib/data/src/schema/auth.ts`).
...
- User task assignment requires active `project_memberships` entry for that user (`server/src/services/issues.ts`).
...
- ensure:
  ...
  - project membership can be created/maintained for this user where needed.

2. `authenticated`
- Better Auth sign-up creates user row.
- bootstrap/admin flow promotes that real user to `instance_admin`.
- first project creation flow should ensure creator membership is active.

## Why This Matters

- `assigneeUserId` validation checks project membership.
...
- `lib/core/src/constants.ts`: define canonical mode/exposure constants.
- `lib/core/src/config-schema.ts`: add mode/exposure/auth URL fields.
...
- ensure first-project creation path grants creator membership.
...
1. `pnpm gitmesh-agents onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm gitmesh-agents doctor` works flagless with mode-aware checks.
