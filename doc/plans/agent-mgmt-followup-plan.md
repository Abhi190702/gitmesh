...
# Agent Management Follow-up Plan (admin Patch + Config Rollback + Issue↔Approval Linking)
...
So even though the admin has enable permission, the route still enforces old self-only patch behavior.

## 1.2 Why comment quality felt wrong

- `playbooks/gitmesh-agents/playbook.md` and `playbooks/gitmesh-agents/references/api-reference.md` do not currently require markdown formatting quality for status comments (links, structure, readable updates).
...
## 2.1 Allow admin to patch other same-project agents

Target behavior:

- Operator: full patch rights.
- admin: can patch agents in same project.
- Other agents: self-only patch unless explicitly granted future permission.

Note:

- Keep project boundary checks strict.
...
- `(project_id, agent_id, revision_number desc)`
...
- unique `(project_id, issue_id, approval_id)`
...
- operator: allow
- agent role `ceo` in same project: allow
- otherwise: self only
...
- `POST /api/projects/:projectId/approvals` supports `issueId` or `issueIds`
- `POST /api/projects/:projectId/agent-enables` supports `sourceIssueId` or `sourceIssueIds`
...
## 6.1 `playbooks/gitmesh-agents/playbook.md`
...
## 6.2 `playbooks/gitmesh-agents-create-agent/playbook.md`

Require:

- include `sourceIssueId` when enable is created from an issue
...
- Fix admin patch authz in agent route
...
- admin can patch pr_review (same project) successfully.
...
- Agent comments in enabling workflow use markdown and include entity links.
