...
Add a single storage subsystem for GitMesh Agents that supports:
...
- Files are always project-scoped and access-controlled.
- API serves attachment bytes through authenticated GitMesh Agents endpoints.
...
- Default local path is under instance root: `~/.gitmesh-agents/instances/<instanceId>/data/storage`.
...
- All object keys include project prefix to preserve hard tenancy boundaries.

## Phase 1: Shared Config + Provider Contract

### Checklist (Per File)

- [ ] `lib/core/src/constants.ts`: add `STORAGE_PROVIDERS` and `StorageProvider` type.
- [ ] `lib/core/src/config-schema.ts`: add `storageConfigSchema` with:
  - provider: `local_disk | s3`
  - localDisk.baseDir
  - s3.bucket, s3.region, s3.endpoint?, s3.prefix?, s3.forcePathStyle?
- [ ] `lib/core/src/index.ts`: export new storage config/types.
...
- `gitmesh-agents onboard` writes a valid `storage` config block by default.
- `gitmesh-agents configure --section storage` can switch between local and s3 modes.
...
- [ ] `lib/data/src/schema/assets.ts`: new generic asset metadata table.
- [ ] `lib/data/src/schema/issue_attachments.ts`: issue-to-asset linking table.
- [ ] `lib/data/src/schema/index.ts`: export new tables.
- [ ] `lib/data/src/migrations/*`: generate migration for both tables and indexes.
- [ ] `lib/core/src/types/issue.ts` (or new asset types file): add `IssueAttachment` type.
- [ ] `lib/core/src/index.ts`: export new types.

### Suggested Columns

- `assets`:
  - `id`, `project_id`, `provider`, `object_key`
  - `content_type`, `byte_size`, `sha256`, `original_filename`
  - `created_by_agent_id`, `created_by_user_id`, timestamps
- `issue_attachments`:
  - `id`, `project_id`, `issue_id`, `asset_id`, `issue_comment_id` (nullable), timestamps

### Acceptance Criteria

- Migration applies cleanly on empty and existing local dev DB.
- Metadata rows are project-scoped and indexed for issue lookup.

## Phase 4: Issue Attachment API

### Checklist (Per File)

- [ ] `lib/core/src/validators/issue.ts`: add schemas for upload/list/delete attachment operations.
- [ ] `server/src/services/issues.ts`: add attachment CRUD helpers with project checks.
- [ ] `server/src/routes/issues.ts`: add endpoints:
  - `POST /projects/:projectId/issues/:issueId/attachments` (multipart)
  - `GET /issues/:issueId/attachments`
  - `GET /attachments/:attachmentId/content`
  - `DELETE /attachments/:attachmentId`
- [ ] `server/src/routes/authz.ts`: reuse/enforce project access for attachment endpoints.
...
- Operator and same-project agents can upload and read attachments per issue permissions.
- Cross-project access is denied even with valid attachment id.
...
- [ ] `lib/core/src/types/index.ts`: ensure attachment types are consumed cleanly in UI.
...
- [ ] `doc/v1-spec.md`: add storage subsystem and issue-attachment endpoint contract.

### Acceptance Criteria

- `gitmesh-agents doctor` reports actionable storage status.
...
- [ ] `server/src/__tests__/issue-attachments.auth.test.ts`: project boundary and permission tests.
