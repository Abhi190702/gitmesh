...
- mention wake to manager/pr_review from `issue_comment_mentioned`
...
2. Extend shared `Issue` type in `lib/core/src/types/issue.ts`.
...
1. Unit tests for `heartbeat.wakeup`:
   ...
   - stale lock recovery
2. Integration tests:
   - comment with `@pr_review` during active assignee run does not create concurrent active run
...
4. Mentioning pr_review during an active issue run does not start pr_review concurrently on that issue.
