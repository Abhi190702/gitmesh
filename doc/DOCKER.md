...
- host port: `3100`
- persistent data dir: `./data/docker-gitmesh-agents`

Optional overrides:

```sh
GITMESH_PORT=3200 GITMESH_DATA_DIR=./data/pc docker compose -f docker-compose.quickstart.yml up --build
```

If you change host port or use a non-local domain, set `GITMESH_PUBLIC_URL` to the external URL you will use in browser/auth flows.
...
services:
  gitmesh-agents:
    environment:
      ...
      GITMESH_PUBLIC_URL: https://desk.koker.net
```

`GITMESH_PUBLIC_URL` is used as the primary source for:
...
- `npx gitmesh-agents onboard --yes` completes
...
GITMESH_DEPLOYMENT_MODE=authenticated GITMESH_DEPLOYMENT_EXPOSURE=private ./scripts/docker-onboard-smoke.sh
```
...
- Smoke script defaults host port to `3131` to avoid conflicts with local GitMesh Agents on `3100`.
