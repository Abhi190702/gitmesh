...
function findContextFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".gitmesh-agents", DEFAULT_CONTEXT_BASENAME);
...
export function resolveContextPath(overridePath?: string): string {
  ...
  if (process.env.GITMESH_CONTEXT) return path.resolve(process.env.GITMESH_CONTEXT);
...
function normalizeProfile(value: unknown): ClientContextProfile {
  ...
  return {
    ...
    projectId: toStringOrUndefined(profile.projectId),
...
export function upsertProfile(
  ...
  if (patch.projectId !== undefined && patch.projectId.trim().length === 0) {
