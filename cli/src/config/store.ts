...
import { gitmeshConfigSchema, type GitmeshConfig } from "./schema.js";
import {
  resolveDefaultConfigPath,
  resolveGitmeshInstanceId,
} from "./home.js";

const PRIMARY_CONFIG_BASENAME = "gitmesh-agents.json";
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidates = [
      path.resolve(currentDir, ".gitmesh-agents", PRIMARY_CONFIG_BASENAME),
      path.resolve(currentDir, ".gitmesh-agents", LEGACY_CONFIG_BASENAME),
    ];
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  const discovered = findConfigFileFromAncestors(process.cwd());
  if (discovered) return discovered;

  const defaultPath = resolveDefaultConfigPath(resolveGitmeshInstanceId());
  if (fs.existsSync(defaultPath)) return defaultPath;

  const legacyPath = defaultPath.replace(
    `${path.sep}${PRIMARY_CONFIG_BASENAME}`,
    `${path.sep}${LEGACY_CONFIG_BASENAME}`,
  );
...
export function readConfig(configPath?: string): GitmeshConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = gitmeshConfigSchema.safeParse(migrated);
  if (!parsed.success) {
