...
import { ApiRequestError, GitmeshApiClient } from "../client/http.js";

describe("GitmeshApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new GitmeshApiClient({
    ...
    expect(headers["x-gitmesh-agents-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new GitmeshApiClient({ apiBase: "http://localhost:3100" });
