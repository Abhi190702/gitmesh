...
import type { SecretProviderModule, SecretMaterial, StoredSecretVersionMaterial } from "./types.js";
...
function resolveMasterKeyFilePath() {
  const fromEnv = process.env.GITMESH_SECRETS_MASTER_KEY_FILE;
...
function loadOrCreateMasterKey(): Buffer {
  const envKeyRaw = process.env.GITMESH_SECRETS_MASTER_KEY;
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    const fromEnv = decodeMasterKey(envKeyRaw);
    if (!fromEnv) {
      throw badRequest(
        "Invalid GITMESH_SECRETS_MASTER_KEY (expected 32-byte base64, 64-char hex, or raw 32-char string)",
      );
    }
    return fromEnv;
  }
...
function asLocalEncryptedMaterial(value: SecretMaterial): LocalEncryptedMaterial {
  const obj = value as unknown as Record<string, unknown>;
  if (
    obj &&
    typeof obj === "object" &&
    obj.scheme === "local_encrypted_v1" &&
    typeof obj.iv === "string" &&
    typeof obj.tag === "string" &&
    typeof obj.ciphertext === "string"
  ) {
    return obj as LocalEncryptedMaterial;
  }
...
export const localEncryptedProvider: SecretProviderModule = {
  ...
  async createVersion(input) {
    ...
    const encrypted = encryptValue(masterKey, input.value);
    return {
      material: encrypted as unknown as SecretMaterial,
      digest: sha256Hex(input.value),
      externalRef: null,
    };
  },
