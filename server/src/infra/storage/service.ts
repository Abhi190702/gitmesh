...
function ensureProjectPrefix(projectId: string, objectKey: string): void {
  const expectedPrefix = `${projectId}/`;
  if (!objectKey.startsWith(expectedPrefix)) {
    throw forbidden("Object does not belong to project");
  }
...
function buildObjectKey(projectId: string, namespace: string, originalFilename: string | null): string {
...
export function createStorageService(provider: StorageProvider): StorageService {
  return {
    ...
    async putFile(input: PutFileInput): Promise<PutFileResult> {
      ...
      const objectKey = buildObjectKey(input.projectId, input.namespace, input.originalFilename);
    ...
    async getObject(projectId: string, objectKey: string) {
      ensureProjectPrefix(projectId, objectKey);
      return provider.getObject({ objectKey });
    },

    async headObject(projectId: string, objectKey: string) {
      ensureProjectPrefix(projectId, objectKey);
      return provider.headObject({ objectKey });
    },

    async deleteObject(projectId: string, objectKey: string) {
      ensureProjectPrefix(projectId, objectKey);
