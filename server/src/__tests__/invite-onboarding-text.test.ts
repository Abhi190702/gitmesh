...
import { buildInviteOnboardingTextDocument } from "../api/access.js";
...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("GitMesh Agents Gateway Gateway Onboarding");
    ...
    expect(text).toContain("Suggested GitMesh Agents base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("gitmeshAgentsApiUrl");
    expect(text).toContain("adapterType \"gateway\"");
    expect(text).toContain("headers.x-gateway-token");
    expect(text).toContain("Do NOT use /v1/responses or /hooks/*");
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.gitmeshAgentsApiUrl");
    expect(text).toContain("~/.gitmesh-gateway/workspace/gitmesh-agents-claimed-api-key.json");
    expect(text).toContain("GITMESH_API_KEY");
