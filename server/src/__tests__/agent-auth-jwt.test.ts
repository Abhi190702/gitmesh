...
describe("agent local JWT", () => {
  const secretEnv = "GITMESH_AGENT_JWT_SECRET";
  const ttlEnv = "GITMESH_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "GITMESH_AGENT_JWT_ISSUER";
  const audienceEnv = "GITMESH_AGENT_JWT_AUDIENCE";
  ...
  it("creates and verifies a token", () => {
    ...
    const token = createLocalAgentJwt("agent-1", "project-1", "claude_local", "run-1");
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    const token = createLocalAgentJwt("agent-1", "project-1", "codex_local", "run-1");

    process.env[issuerEnv] = "gitmesh-agents";
    process.env[audienceEnv] = "gitmesh-agents-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
