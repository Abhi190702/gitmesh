...
import { projects, projectMemberships, instanceUserRoles } from "@gitmesh/data";
import type { DeploymentMode } from "@gitmesh/core";
...
export async function claimBoardOwnership(
  ...
  await db.transaction(async (tx) => {
    ...
    const allProjects = await tx.select({ id: projects.id }).from(projects);
      ...
      const existing = await tx
        ...
        .where(
          and(
            ...
            eq(projectMemberships.principalId, opts.userId),
          ),
        )
        .then((rows) => rows[0] ?? null);
