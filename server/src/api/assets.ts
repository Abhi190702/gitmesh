...
import { createAssetImageMetadataSchema } from "@gitmesh/core";
import type { StorageService } from "../infra/storage/types.js";
import { assetService, logActivity } from "../core/index.js";
import { assertProjectAccess, getActorInfo } from "./authz.js";

const MAX_ASSET_IMAGE_BYTES = Number(process.env.GITMESH_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;
...
export function assetRoutes(db: Db, storage: StorageService) {
  ...
  router.post("/projects/:projectId/assets/images", async (req, res) => {
    ...
    const asset = await svc.create(projectId, {
  ...
  router.get("/assets/:assetId/content", async (req, res, next) => {
    ...
    assertProjectAccess(req, asset.projectId);

    const object = await storage.getObject(asset.projectId, asset.objectKey);
