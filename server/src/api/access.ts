...
import { and, eq, isNull, desc } from "@gitmesh/data";
...
const INVITE_TOKEN_PREFIX = "gmesh_invite_";
const INVITE_TOKEN_ALPHABET = "abcdefghijklmnopqrstuvwxyz0123456789";
const INVITE_TOKEN_SUFFIX_LENGTH = 8;
const INVITE_TOKEN_MAX_RETRIES = 5;
const PROJECT_INVITE_TTL_MS = 10 * 60 * 1000;
...
function createClaimSecret() {
  return `gmesh_claim_${randomBytes(24).toString("hex")}`;
}

export function projectInviteExpiresAt(nowMs: number = Date.now()) {
  return new Date(nowMs + PROJECT_INVITE_TTL_MS);
}
...
function readPlaybookMarkdown(playbookName: string): string | null {
  const normalized = playbookName.trim().toLowerCase();
  if (normalized !== "core" && normalized !== "adapter-dev")
    return null;
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
    path.resolve(moduleDir, "../../playbooks", normalized, "playbook.md"), // published: dist/routes/ -> <pkg>/playbooks/
    path.resolve(process.cwd(), "playbooks", normalized, "playbook.md"), // cwd (e.g. monorepo root)
    path.resolve(moduleDir, "../../../playbooks", normalized, "playbook.md") // dev: src/routes/ -> repo root/playbooks/
  ];
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.gitmeshAgentsApiUrl)) {
    const legacyGitmeshAgentsApiUrl = nonEmptyTrimmedString(input.gitmeshAgentsApiUrl);
    if (legacyGitmeshAgentsApiUrl) merged.gitmeshAgentsApiUrl = legacyGitmeshAgentsApiUrl;
  }
  const mergedHeaders = normalizeHeaderMap(merged.headers) ?? {};

  const inboundGatewayAuthHeader = nonEmptyTrimmedString(
    input.inboundGatewayAuthHeader
  );
  const inboundGatewayTokenHeader = nonEmptyTrimmedString(
    input.inboundGatewayTokenHeader
  );
  if (
    inboundGatewayTokenHeader &&
    !headerMapHasKeyIgnoreCase(mergedHeaders, "x-gateway-token")
  ) {
    mergedHeaders["x-gateway-token"] = inboundGatewayTokenHeader;
  }
  if (
    inboundGatewayAuthHeader &&
    !headerMapHasKeyIgnoreCase(mergedHeaders, "x-gateway-auth")
  ) {
    mergedHeaders["x-gateway-auth"] = inboundGatewayAuthHeader;
  }
  ...
  const discoveredToken =
    headerMapGetIgnoreCase(mergedHeaders, "x-gateway-token") ??
    headerMapGetIgnoreCase(mergedHeaders, "x-gateway-auth") ??
  ...
  if (
    ...
    mergedHeaders["x-gateway-token"] = discoveredToken;
  }
...
export function canReplayGatewayInviteAccept(input: {
  ...
  if (
    ...
    input.existingJoinRequest.adapterType !== "gateway"
  ) {
    return false;
  }
  return (
...
function summarizeGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  const gatewayTokenValue = headers
    ? headerMapGetIgnoreCase(headers, "x-gateway-token") ??
    headerMapGetIgnoreCase(headers, "x-gateway-auth") ??
    tokenFromAuthorizationHeader(
  ...
  return {
    ...
    gitmeshAgentsApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.gitmeshAgentsApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  if (!isPlainObject(input.defaultsPayload)) {
    diagnostics.push({
      code: "gateway_defaults_missing",
      level: "warn",
      message:
        "No Gateway gateway config was provided in agentDefaultsPayload.",
      hint:
        "Include agentDefaultsPayload.url and headers.x-gateway-token for Gateway gateway joins."
    });
    fatalErrors.push(
      "agentDefaultsPayload is required for adapterType=gateway"
    );
    return {
  ...
  if (!rawGatewayUrl) {
    diagnostics.push({
      ...
      level: "warn",
      message: "Gateway gateway URL is missing.",
    ...
    try {
      ...
      if (gatewayUrl.protocol !== "ws:" && gatewayUrl.protocol !== "wss:") {
        diagnostics.push({
          code: "gateway_url_protocol",
          level: "warn",
          message: `Gateway gateway URL must use ws:// or wss:// (got ${gatewayUrl.protocol}).`
        });
        fatalErrors.push(
          "agentDefaultsPayload.url must use ws:// or wss:// for gateway"
        );
      } else {
        normalized.url = gatewayUrl.toString();
        diagnostics.push({
          code: "gateway_url_configured",
          level: "info",
      ...
      diagnostics.push({
        ...
        level: "warn",
        message: `Invalid Gateway gateway URL: ${rawGatewayUrl}`
      });
  ...
  if (gatewayToken && !headerMapHasKeyIgnoreCase(headers, "x-gateway-token")) {
  ...
  if (!gatewayToken) {
    diagnostics.push({
      code: "gateway_auth_header_missing",
      level: "warn",
      message: "Gateway auth token is missing from agent defaults.",
      hint:
        "Set agentDefaultsPayload.headers.x-gateway-token (or legacy x-gateway-auth)."
    });
    fatalErrors.push(
      "agentDefaultsPayload.headers.x-gateway-token (or x-gateway-auth) is required"
    );
  } else if (gatewayToken.trim().length < 16) {
    diagnostics.push({
      code: "gateway_auth_header_too_short",
      level: "warn",
      message: `Gateway auth token appears too short (${gatewayToken.trim().length} chars).`,
      hint:
        "Use the full gateway auth token from ~/.gitmesh-gateway/config.json (typically long random string)."
    });
    fatalErrors.push(
      "agentDefaultsPayload.headers.x-gateway-token is too short; expected a full gateway token"
    );
  } else {
    diagnostics.push({
      code: "gateway_auth_header_configured",
      level: "info",
  ...
  if (configuredDevicePrivateKeyPem) {
    ...
    diagnostics.push({
      code: "gateway_device_key_configured",
      level: "info",
      message:
    ...
    try {
      ...
      diagnostics.push({
        code: "gateway_device_key_generated",
        level: "info",
        message:
      ...
      diagnostics.push({
        code: "gateway_device_key_generate_failed",
        level: "warn",
  ...
  const rawGitmeshAgentsApiUrl =
    typeof defaults.gitmeshAgentsApiUrl === "string"
      ? defaults.gitmeshAgentsApiUrl.trim()
      : "";
  if (rawGitmeshAgentsApiUrl) {
    try {
      const parsedGitmeshAgentsApiUrl = new URL(rawGitmeshAgentsApiUrl);
      if (
        parsedGitmeshAgentsApiUrl.protocol !== "http:" &&
        parsedGitmeshAgentsApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "gateway_gitmesh-agents_api_url_protocol",
          level: "warn",
          message: `gitmeshAgentsApiUrl must use http:// or https:// (got ${parsedGitmeshAgentsApiUrl.protocol}).`
        });
      } else {
        normalized.gitmeshAgentsApiUrl = parsedGitmeshAgentsApiUrl.toString();
        diagnostics.push({
          code: "gateway_gitmesh-agents_api_url_configured",
          level: "info",
          message: `gitmeshAgentsApiUrl set to ${parsedGitmeshAgentsApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "gateway_gitmesh-agents_api_url_invalid",
        level: "warn",
        message: `Invalid gitmeshAgentsApiUrl: ${rawGitmeshAgentsApiUrl}`
      });
    }
  }
...
function toInviteSummaryResponse(
  ...
  return {
    ...
    onboardingTextUrl: baseUrl
      ...
      : onboardingTextPath,
    skillIndexPath: "/api/playbooks/index",
    skillIndexUrl: baseUrl
      ? `${baseUrl}/api/playbooks/index`
      : "/api/playbooks/index",
    inviteMessage
  };
}
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      code: "gateway_onboarding_api_loopback",
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote Gateway agents cannot reach localhost on your GitMesh Agents host.",
  ...
  if (
    ...
    diagnostics.push({
      code: "gateway_onboarding_private_loopback_bind",
      level: "warn",
      message: "GitMesh Agents is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      code: "gateway_onboarding_private_host_not_allowed",
      level: "warn",
      message: `Onboarding host "${apiHost}" is not in allowed hostnames for authenticated/private mode.`,
      hint: `Run pnpm gitmesh-agents allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/playbooks/core";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as a Gateway agent, save your one-time claim secret, wait for maintainer approval, then claim your API key. Save the claim response token to ~/.gitmesh-gateway/workspace/gitmesh-agents-claimed-api-key.json and load GITMESH_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='gateway', set agentDefaultsPayload.url to your ws:// or wss:// Gateway gateway endpoint, and include agentDefaultsPayload.headers.x-gateway-token (or legacy x-gateway-auth).",
      inviteMessage: extractInviteMessage(invite),
      recommendedAdapterType: "gateway",
      requiredFields: {
        requestType: "agent",
        agentName: "Display name for this agent",
        adapterType: "Use 'gateway' for Gateway Gateway agents",
        capabilities: "Optional capability summary",
        agentDefaultsPayload:
          "Adapter config for Gateway gateway. MUST include url (ws:// or wss://) and headers.x-gateway-token (or legacy x-gateway-auth). Optional fields: gitmeshAgentsApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          opts.deploymentMode === "authenticated" &&
            ...
            ? "If Gateway runs on another machine, ensure the GitMesh Agents hostname is reachable and allowed via `pnpm gitmesh-agents allowed-hostname <host>`."
            : "Ensure Gateway can reach this GitMesh Agents API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.gitmesh-gateway/playbooks/core/playbook.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # GitMesh Agents Gateway Gateway Onboarding
  ...
  appendBlock(`
    ...
    Get the Gateway gateway auth token (THIS MUST BE SENT)
    Token lives in:

    ~/.gitmesh-gateway/config.json -> gateway.auth.token
    Extract:

    TOKEN="$(node -p 'require(process.env.HOME+\"/.gitmesh-gateway/config.json\").gateway.auth.token')"
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentName: "Gateway",
        adapterType: "gateway",
        capabilities: "Gateway agent adapter",
        agentDefaultsPayload: {
    ...
    IMPORTANT: You MUST include agentDefaultsPayload.headers.x-gateway-token with your gateway token.
    Legacy x-gateway-auth is also accepted, but x-gateway-token is preferred.
    Use adapterType "gateway" and a ws:// or wss:// gateway URL.
    Pairing mode requirement:
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, GitMesh Agents generates and persists one during join so pairing approvals are stable.
    - You may set disableDeviceAuth=true only for special environments that cannot support pairing.
    - First run may return "pairing required" once; approve the pending pairing request in Gateway, then retry.
    Do NOT use /v1/responses or /hooks/* in this gateway join flow.

    Body (JSON):
    {
      "requestType": "agent",
      "agentName": "My Gateway Agent",
      "adapterType": "gateway",
      "capabilities": "Optional summary",
      "agentDefaultsPayload": {
        "url": "wss://your-gateway.example",
        "gitmeshAgentsApiUrl": "https://gitmesh-agents-hostname-your-agent-can-reach:3100",
        "headers": { "x-gateway-token": "replace-me" },
        "waitTimeoutMs": 120000,
        "sessionKeyStrategy": "issue",
        "role": "operator",
        "scopes": ["operator.admin"]
    ...
    ## Step 2: Wait for maintainer approval
    The maintainer approves the join request in GitMesh Agents before key claim is allowed.
    ...
    - ~/.gitmesh-gateway/workspace/gitmesh-agents-claimed-api-key.json
    chmod 600 ~/.gitmesh-gateway/workspace/gitmesh-agents-claimed-api-key.json

    And set the GITMESH_API_KEY and GITMESH_API_URL in your environment variables as specified here:
    https://docs.gitmesh.dev/help/environment
    ...
    Then set GITMESH_API_KEY and GITMESH_API_URL from the saved token field for every heartbeat run.

    Important:
    - claim secrets expire
    - claim secrets are single-use
    - claim fails before maintainer approval

    ## Step 4: Install GitMesh Agents skill in Gateway
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your GITMESH_API_URL to the top of your skill and note the path to your GITMESH_API_URL
    ...
    "Ensure GitMesh Agents is reachable from your Gateway runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested GitMesh Agents base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.gitmeshAgentsApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm gitmesh-agents allowed-hostname <host>
      - then restart GitMesh Agents and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@gitmesh.local";
...
export function accessRoutes(
  ...
  async function assertCanGenerateGatewayInvitePrompt(
    req: Request,
    projectId: string
  ) {
    assertProjectAccess(req, projectId);
    if (req.actor.type === "agent") {
      ...
      if (actorAgent.role !== "admin") {
        throw forbidden("Only CEO agents can generate Gateway invite prompts");
      }
      return;
    }
    if (req.actor.type !== "operator") throw unauthorized();
    if (isLocalImplicit(req)) return;
    const allowed = await access.canUser(projectId, req.actor.userId, "users:invite");
    if (!allowed) throw forbidden("Permission denied");
  }

  async function createProjectInviteForProject(input: {
    req: Request;
    projectId: string;
  ...
  router.get("/playbooks/index", (_req, res) => {
    res.json({
      playbooks: [
        { name: "core", path: "/api/playbooks/core" },
        {
          name: "adapter-dev",
          path: "/api/playbooks/adapter-dev"
        }
      ]
    });
  });

  router.get("/playbooks/:playbookName", (req, res) => {
    const playbookName = (req.params.playbookName as string).trim().toLowerCase();
    const markdown = readPlaybookMarkdown(playbookName);
    if (!markdown) throw notFound("Playbook not found");
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      await assertProjectPermission(req, projectId, "users:invite");
      const { token, created, normalizedAgentMessage } =
        await createProjectInviteForProject({
          req,
          projectId,
  ...
  router.post(
    ...
    validate(createGatewayInvitePromptSchema),
    async (req, res) => {
      const projectId = req.params.projectId as string;
      await assertCanGenerateGatewayInvitePrompt(req, projectId);
      const { token, created, normalizedAgentMessage } =
        await createProjectInviteForProject({
          req,
          projectId,
      ...
      await logActivity(db, {
        ...
        actorId:
          req.actor.type === "agent"
            ? req.actor.agentId ?? "unknown-agent"
            : req.actor.userId ?? "operator",
        action: "invite.gateway_prompt_created",
        entityType: "invite",
        entityId: created.id,
        details: {
          inviteType: created.inviteType,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (!projectId) throw conflict("Invite is missing project scope");
      if (
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
            ...
            gitmeshAgentsApiUrl: req.body.gitmeshAgentsApiUrl ?? null,
            inboundGatewayAuthHeader: req.header("x-gateway-auth") ?? null,
            inboundGatewayTokenHeader: req.header("x-gateway-token") ?? null
          })
          : null;
      ...
      if (requestType === "agent" && adapterType === "gateway") {
        logger.info(
          {
            inviteId: invite.id,
            ...
            normalizedAgentDefaults: summarizeGatewayDefaultsForLog(
              joinDefaults.normalized
            )
          },
          "invite accept normalized Gateway gateway defaults"
        );
      }
      ...
      if (requestType === "agent" && adapterType === "gateway") {
        const expectedDefaults = summarizeGatewayDefaultsForLog(
          joinDefaults.normalized
        );
        const persistedDefaults = summarizeGatewayDefaultsForLog(
        ...
        if (
          expectedDefaults.gitmeshAgentsApiUrl &&
          !persistedDefaults.gitmeshAgentsApiUrl
        ) {
          missingPersistedFields.push("gitmeshAgentsApiUrl");
        }
        if (expectedDefaults.gatewayToken && !persistedDefaults.gatewayToken) {
          missingPersistedFields.push("headers.x-gateway-token");
        }
        if (
        ...
        logger.info(
          ...
          "invite accept persisted Gateway gateway join request"
        );
        ...
        if (missingPersistedFields.length > 0) {
          logger.warn(
            ...
            "invite accept detected missing persisted Gateway gateway defaults"
          );
        }
      }
  ...
  router.post("/invites/:inviteId/revoke", async (req, res) => {
    ...
    if (invite.inviteType === "bootstrap_ceo") {
      ...
      if (!invite.projectId) throw conflict("Invite is missing project scope");
      await assertProjectPermission(req, invite.projectId, "users:invite");
    }
  ...
  router.get("/projects/:projectId/join-requests", async (req, res) => {
    const projectId = req.params.projectId as string;
    await assertProjectPermission(req, projectId, "joins:approve");
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (existing.requestType === "human") {
        ...
        const existingAgents = await agents.list(projectId);
        const managerId = resolveJoinRequestAgentManagerId(existingAgents);
        if (!managerId) {
          throw conflict(
            "Join request cannot be approved because this project has no active CEO"
          );
        }
        ...
        const created = await agents.create(projectId, {
          name: agentName,
          role: "general",
          title: null,
          status: "idle",
  ...
  router.get("/projects/:projectId/members", async (req, res) => {
    const projectId = req.params.projectId as string;
    await assertProjectPermission(req, projectId, "users:manage_permissions");
    const members = await access.listMembers(projectId);
    res.json(members);
  });
  ...
  router.get("/admin/users/:userId/project-access", async (req, res) => {
    await assertInstanceAdmin(req);
    const userId = req.params.userId as string;
    const memberships = await access.listUserProjectAccess(userId);
    res.json(memberships);
  });

  router.put(
    "/admin/users/:userId/project-access",
    validate(updateUserProjectAccessSchema),
    async (req, res) => {
      await assertInstanceAdmin(req);
      const userId = req.params.userId as string;
      const memberships = await access.setUserProjectAccess(
        userId,
        req.body.projectIds ?? []
      );
      res.json(memberships);
