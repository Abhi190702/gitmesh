import { and, desc, eq } from "@gitmesh/data";
import type { Db } from "@gitmesh/data";
import { projectSecrets, projectSecretVersions } from "@gitmesh/data";
import type { AgentEnvConfig, EnvBinding, SecretProvider } from "@gitmesh/core";
import type { SecretMaterial } from "../infra/secrets/types.js";
import { envBindingSchema } from "@gitmesh/core";
import { conflict, notFound, unprocessable } from "../errors.js";
import { getSecretProvider, listSecretProviders } from "../infra/secrets/provider-registry.js";
...
export function secretService(db: Db) {
  ...
  async function getByName(projectId: string, name: string) {
    return db
      .select()
      .from(projectSecrets)
  ...
  async function getSecretVersion(secretId: string, version: number) {
    return db
      ...
      .where(
        and(
          eq(projectSecretVersions.secretId, secretId),
          eq(projectSecretVersions.version, version),
        ),
      )
      .then((rows) => rows[0] ?? null);
  }

  async function assertSecretInProject(projectId: string, secretId: string) {
  ...
  async function resolveSecretValue(
    ...
    const secret = await assertSecretInProject(projectId, secretId);
    ...
    return provider.resolveVersion({
      material: versionRow.material as unknown as SecretMaterial,
  ...
  async function normalizeEnvConfig(
    ...
    for (const [key, rawBinding] of Object.entries(record)) {
      ...
      await assertSecretInProject(projectId, binding.secretId);
      normalized[key] = {
        type: "secret_ref",
        secretId: binding.secretId,
  ...
  async function normalizeAdapterConfigForPersistenceInternal(
    ...
    normalized.env = await normalizeEnvConfig(projectId, adapterConfig.env, opts);
    return normalized;
  }
  ...
  return {
    ...
    create: async (
      ...
      const existing = await getByName(projectId, input.name);
      ...
      return db.transaction(async (tx) => {
        ...
        await tx.insert(projectSecretVersions).values({
          secretId: secret.id,
          version: 1,
          material: prepared.material as unknown as Record<string, unknown>,
          valueSha256: prepared.digest,
          createdByAgentId: actor?.agentId ?? null,
          createdByUserId: actor?.userId ?? null,
        });
    ...
    update: async (
      ...
      if (patch.name && patch.name !== secret.name) {
        const duplicate = await getByName(secret.projectId, patch.name);
    ...
    /**
     * Resolve a stored secret value to its plaintext form. Used by
     * subsystems that hold a known secret name (attestation signing key,
     * GitHub token, etc.) and need the underlying value at request time.
     */
    resolveSecretValue: async (
      projectId: string,
      secretId: string,
      version: number | "latest",
    ): Promise<string> => resolveSecretValue(projectId, secretId, version),

    normalizeAdapterConfigForPersistence: async (
      projectId: string,
      adapterConfig: Record<string, unknown>,
      opts?: { strictMode?: boolean },
    ) => normalizeAdapterConfigForPersistenceInternal(projectId, adapterConfig, opts),
    ...
    resolveEnvBindings: async (projectId: string, envValue: unknown): Promise<{ env: Record<string, string>; secretKeys: Set<string> }> => {
      const record = asRecord(envValue);
      ...
      for (const [key, rawBinding] of Object.entries(record)) {
        ...
        if (binding.type === "plain") {
          ...
          resolved[key] = await resolveSecretValue(projectId, binding.secretId, binding.version);
          secretKeys.add(key);
        }
      }
    ...
    resolveAdapterConfigForRuntime: async (projectId: string, adapterConfig: Record<string, unknown>): Promise<{ config: Record<string, unknown>; secretKeys: Set<string> }> => {
      ...
      for (const [key, rawBinding] of Object.entries(record)) {
        ...
        if (binding.type === "plain") {
          ...
          env[key] = await resolveSecretValue(projectId, binding.secretId, binding.version);
          secretKeys.add(key);
        }
      }
      resolved.env = env;
