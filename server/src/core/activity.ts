import { and, desc, eq, isNull, or, sql } from "@gitmesh/data";
import type { Db } from "@gitmesh/data";
import { activityLog, heartbeatRuns, issues } from "@gitmesh/data";
...
export function activityService(db: Db) {
  ...
  return {
    list: (filters: ActivityFilters) => {
      const conditions = [eq(activityLog.projectId, filters.projectId)];
    ...
    runsForIssue: (projectId: string, issueId: string) =>
      db
        .select({
          ...
          and(
            ...
            or(
              ...
              sql`exists (
                select 1
                from ${activityLog}
                where ${activityLog.projectId} = ${projectId}
    ...
    issuesForRun: async (runId: string) => {
      const run = await db
        .select({
          projectId: heartbeatRuns.projectId,
      ...
      const fromActivity = await db
        ...
        .where(
          and(
            eq(activityLog.projectId, run.projectId),
      ...
      const fromContext = await db
        ...
        .where(
          and(
            eq(issues.projectId, run.projectId),
