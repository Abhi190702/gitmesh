import { and, asc, desc, eq, inArray, isNull, or, sql } from "@gitmesh/data";
...
import { extractProjectMentionIds } from "@gitmesh/core";
...
function touchedByUserCondition(projectId: string, userId: string) {
  return sql<boolean>`
    (
      ...
      OR EXISTS (
        ...
        WHERE ${issueReadStates.issueId} = ${issues.id}
          AND ${issueReadStates.projectId} = ${projectId}
      ...
      OR EXISTS (
        ...
        WHERE ${issueComments.issueId} = ${issues.id}
          AND ${issueComments.projectId} = ${projectId}
...
function myLastCommentAtExpr(projectId: string, userId: string) {
...
function myLastReadAtExpr(projectId: string, userId: string) {
  return sql<Date | null>`
    (
...
function myLastTouchAtExpr(projectId: string, userId: string) {
  const myLastCommentAt = myLastCommentAtExpr(projectId, userId);
  const myLastReadAt = myLastReadAtExpr(projectId, userId);
  return sql<Date | null>`
    GREATEST(
...
function unreadForUserCondition(projectId: string, userId: string) {
  const touchedCondition = touchedByUserCondition(projectId, userId);
  const myLastTouchAt = myLastTouchAtExpr(projectId, userId);
  return sql<boolean>`
    (
      ${touchedCondition}
      AND EXISTS (
...
export function issueService(db: Db) {
  async function assertAssignableAgent(projectId: string, agentId: string) {
    ...
    if (assignee.projectId !== projectId) {
      throw unprocessable("Assignee must belong to same project");
    }
  ...
  async function assertAssignableUser(projectId: string, userId: string) {
    const membership = await db
      ...
      .where(
        and(
          ...
          eq(projectMemberships.principalId, userId),
  ...
  async function assertValidLabelIds(projectId: string, labelIds: string[], dbOrTx: any = db) {
    ...
    if (existing.length !== new Set(labelIds).size) {
      throw unprocessable("One or more labels are invalid for this project");
    }
  }
  ...
  async function syncIssueLabels(
    ...
    await assertValidLabelIds(projectId, deduped, dbOrTx);
  ...
  return {
    list: async (projectId: string, filters?: IssueFilters) => {
      ...
      if (touchedByUserId) {
        conditions.push(touchedByUserCondition(projectId, touchedByUserId));
      }
      if (unreadForUserId) {
        conditions.push(unreadForUserCondition(projectId, unreadForUserId));
      }
      ...
      if (filters?.labelId) {
        const labeledIssueIds = await db
          ...
          .from(issueLabels)
          .where(and(eq(issueLabels.projectId, projectId), eq(issueLabels.labelId, filters.labelId)));
      ...
      const statsRows = await db
        ...
        .where(
          and(
            eq(issueComments.projectId, projectId),
      ...
      const readRows = await db
        ...
        .where(
          and(
            eq(issueReadStates.projectId, projectId),
    ...
    countUnreadTouchedByUser: async (projectId: string, userId: string, status?: string) => {
      const conditions = [
        eq(issues.projectId, projectId),
        isNull(issues.hiddenAt),
        unreadForUserCondition(projectId, userId),
      ];
      if (status) {
    ...
    markRead: async (projectId: string, issueId: string, userId: string, readAt: Date = new Date()) => {
      const now = new Date();
      const [row] = await db
        ...
        .onConflictDoUpdate({
          target: [issueReadStates.projectId, issueReadStates.issueId, issueReadStates.userId],
          set: {
            lastReadAt: readAt,
            updatedAt: now,
          },
    ...
    create: async (
      projectId: string,
      data: Omit<typeof issues.$inferInsert, "projectId"> & { labelIds?: string[] },
    ) => {
      ...
      if (data.assigneeAgentId) {
        await assertAssignableAgent(projectId, data.assigneeAgentId);
      }
      if (data.assigneeUserId) {
        await assertAssignableUser(projectId, data.assigneeUserId);
      }
      ...
      return db.transaction(async (tx) => {
        ...
        const identifier = `${project.issuePrefix}-${issueNumber}`;

        const values = { ...issueData, projectId, issueNumber, identifier } as typeof issues.$inferInsert;
        ...
        if (inputLabelIds) {
          await syncIssueLabels(issue.id, projectId, inputLabelIds, tx);
        }
    ...
    update: async (id: string, data: Partial<typeof issues.$inferInsert> & { labelIds?: string[] }) => {
      ...
      if (issueData.assigneeAgentId) {
        await assertAssignableAgent(existing.projectId, issueData.assigneeAgentId);
      }
      if (issueData.assigneeUserId) {
        await assertAssignableUser(existing.projectId, issueData.assigneeUserId);
      }
      ...
      return db.transaction(async (tx) => {
        ...
        if (nextLabelIds !== undefined) {
          await syncIssueLabels(updated.id, existing.projectId, nextLabelIds, tx);
        }
    ...
    checkout: async (id: string, agentId: string, expectedStatuses: string[], checkoutRunId: string | null) => {
      const issueProject = await db
        .select({ projectId: issues.projectId })
        .from(issues)
        .where(eq(issues.id, id))
        .then((rows) => rows[0] ?? null);
      if (!issueProject) throw notFound("Issue not found");
      await assertAssignableAgent(issueProject.projectId, agentId);
    ...
    createLabel: async (projectId: string, data: Pick<typeof labels.$inferInsert, "name" | "color">) => {
    ...
    createAttachment: async (input: {
      ...
      if (input.issueCommentId) {
        const comment = await db
          .select({ id: issueComments.id, projectId: issueComments.projectId, issueId: issueComments.issueId })
          .from(issueComments)
          .where(eq(issueComments.id, input.issueCommentId))
          .then((rows) => rows[0] ?? null);
        if (!comment) throw notFound("Issue comment not found");
        if (comment.projectId !== issue.projectId || comment.issueId !== issue.id) {
          throw unprocessable("Attachment comment must belong to same issue and project");
        }
      }
    ...
    findMentionedAgents: async (projectId: string, body: string) => {
      const re = /\B@([^\s@,!?.]+)/g;
    ...
    findMentionedProjectIds: async (issueId: string) => {
      ...
      const rows = await db
        ...
        .where(
          and(
            eq(subprojects.projectId, issue.projectId),
            inArray(subprojects.id, [...mentionedIds]),
          ),
        );
    ...
    getAncestors: async (issueId: string) => {
      ...
      if (projectIds.length > 0) {
        ...
        const rows = await db.select({
          id: subprojects.id, name: subprojects.name, description: subprojects.description,
          status: subprojects.status, goalId: subprojects.goalId,
        }).from(subprojects).where(inArray(subprojects.id, projectIds));
        for (const r of rows) {
          const projectWorkspaceRows = workspaceMap.get(r.id) ?? [];
          const workspaces = projectWorkspaceRows.map((workspace) => ({
            id: workspace.id,
            projectId: workspace.projectId,
            subprojectId: workspace.subprojectId,
            name: workspace.name,
            cwd: workspace.cwd,
    ...
    staleCount: async (projectId: string, minutes = 60) => {
