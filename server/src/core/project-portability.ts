...
import type {
  ...
  ProjectPortabilityExport,
  ProjectPortabilityExportResult,
  ProjectPortabilityImport,
...
import { normalizeAgentUrlKey, portabilityManifestSchema } from "@gitmesh/core";
import { notFound, unprocessable } from "../errors.js";
import { accessService } from "./access.js";
import { agentService } from "./agents.js";
import { projectService } from "./projects.js";

const DEFAULT_INCLUDE: ProjectPortabilityInclude = {
  project: true,
  agents: true,
};

const DEFAULT_COLLISION_STRATEGY: ProjectPortabilityCollisionStrategy = "rename";
...
type ImportPlanInternal = {
  preview: ProjectPortabilityPreviewResult;
  source: ResolvedSource;
  include: ProjectPortabilityInclude;
  collisionStrategy: ProjectPortabilityCollisionStrategy;
  selectedAgents: ProjectPortabilityAgentManifestEntry[];
};
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  gateway: [
    ...
    { path: ["sessionKey"], value: "gitmesh-agents" },
...
function normalizeInclude(input?: Partial<ProjectPortabilityInclude>): ProjectPortabilityInclude {
  return {
    project: input?.project ?? DEFAULT_INCLUDE.project,
...
function normalizePortableEnv(
  agentSlug: string,
  envValue: unknown,
  requiredSecrets: ProjectPortabilityManifest["requiredSecrets"],
) {
...
function renderProjectAgentsSection(agentSummaries: Array<{ slug: string; name: string }>) {
  const lines = ["# Agents", ""];
...
function dedupeRequiredSecrets(values: ProjectPortabilityManifest["requiredSecrets"]) {
  const seen = new Set<string>();
  const out: ProjectPortabilityManifest["requiredSecrets"] = [];
...
async function readAgentInstructions(agent: AgentLike): Promise<{ body: string; warning: string | null }> {
  ...
  if (instructionsFilePath) {
    const workspaceCwd = asString(process.env.GITMESH_WORKSPACE_CWD);
...
export function projectPortabilityService(db: Db) {
  const projects = projectService(db);
  const agents = agentService(db);
  const access = accessService(db);

  async function resolveSource(source: ProjectPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    if (source.type === "url") {
      ...
      if (manifest.project?.path) {
        const projectPath = ensureMarkdownPath(manifest.project.path);
        files[projectPath] = await fetchText(new URL(projectPath, base).toString());
      }
    ...
    let ref = parsed.ref;
    const manifestRelativePath = [parsed.basePath, "gitmesh-agents.manifest.json"].filter(Boolean).join("/");
    let manifest: ProjectPortabilityManifest | null = null;
    const warnings: string[] = [];
    try {
    ...
    if (!manifest) {
      throw new Error("Failed to fetch or parse manifest from GitHub.");
    }

    const files: Record<string, string> = {};
    if (manifest.project?.path) {
      files[manifest.project.path] = await fetchText(
        resolveRawGitHubUrl(parsed.owner, parsed.repo, ref, [parsed.basePath, manifest.project.path].filter(Boolean).join("/")),
      );
    }
    for (const agent of manifest.agents) {
  ...
  async function exportBundle(
    projectId: string,
    input: ProjectPortabilityExport,
  ): Promise<ProjectPortabilityExportResult> {
    ...
    const files: Record<string, string> = {};
    const warnings: string[] = [];
    const requiredSecrets: ProjectPortabilityManifest["requiredSecrets"] = [];
    const generatedAt = new Date().toISOString();

    const manifest: ProjectPortabilityManifest = {
      schemaVersion: 1,
      generatedAt,
      source: {
    ...
    const allAgentRows = include.agents ? await agents.list(projectId, { includeTerminated: true }) : [];
    ...
    if (include.project) {
      ...
      const projectAgentSummaries = agentRows.map((agent) => ({
      ...
      files[projectPath] = buildMarkdown(
        {
          ...
          requireOperatorApprovalForNewAgents: project.requireOperatorApprovalForNewAgents,
        },
        renderProjectAgentsSection(projectAgentSummaries),
      );
      manifest.project = {
        path: projectPath,
        name: project.name,
        ...
        forgeProvider: (project.forgeProvider as "github" | "gitlab" | "forgejo" | null) ?? null,
        forgeOwner: project.forgeOwner ?? null,
        forgeRepo: project.forgeRepo ?? null,
      };
    }
  ...
  async function buildPreview(input: ProjectPortabilityPreview): Promise<ImportPlanInternal> {
    const include = normalizeInclude(input.include);
    ...
    if (include.project && !manifest.project) {
      errors.push("Manifest does not include project metadata.");
    }
    ...
    if (input.target.mode === "existing_project") {
      const targetProject = await projects.getById(input.target.projectId);
      if (!targetProject) throw notFound("Target project not found");
      targetProjectId = targetProject.id;
      targetProjectName = targetProject.name;
    }

    const agentPlans: ProjectPortabilityPreviewAgentPlan[] = [];
    const existingSlugToAgent = new Map<string, { id: string; name: string }>();
    const existingSlugs = new Set<string>();

    if (input.target.mode === "existing_project") {
      const existingAgents = await agents.list(input.target.projectId);
    ...
    const preview: ProjectPortabilityPreviewResult = {
      include,
      targetProjectId,
      targetProjectName,
      collisionStrategy,
      ...
      plan: {
        projectAction: input.target.mode === "new_project"
          ? "create"
          : include.project
            ? "update"
            : "none",
  ...
  async function previewImport(input: ProjectPortabilityPreview): Promise<ProjectPortabilityPreviewResult> {
    const plan = await buildPreview(input);
    return plan.preview;
  }

  async function importBundle(
    input: ProjectPortabilityImport,
    actorUserId: string | null | undefined,
  ): Promise<ProjectPortabilityImportResult> {
    const plan = await buildPreview(input);
    ...
    let projectAction: "created" | "updated" | "unchanged" = "unchanged";

    if (input.target.mode === "new_project") {
      const projectName =
        asString(input.target.newProjectName) ??
        sourceManifest.project?.name ??
        sourceManifest.source?.projectName ??
        "Imported Project";
      const created = await projects.create({
        name: projectName,
        description: include.project ? (sourceManifest.project?.description ?? null) : null,
        brandColor: include.project ? (sourceManifest.project?.brandColor ?? null) : null,
        requireOperatorApprovalForNewAgents: include.project
          ? (sourceManifest.project?.requireOperatorApprovalForNewAgents ?? true)
          : true,
      });
      ...
      targetProject = await projects.getById(input.target.projectId);
      if (!targetProject) throw notFound("Target project not found");
      if (include.project && sourceManifest.project) {
        const updated = await projects.update(targetProject.id, {
          name: sourceManifest.project.name,
          description: sourceManifest.project.description,
          brandColor: sourceManifest.project.brandColor,
          requireOperatorApprovalForNewAgents: sourceManifest.project.requireOperatorApprovalForNewAgents,
        });
        targetProject = updated ?? targetProject;
    ...
    const resultAgents: ProjectPortabilityImportResult["agents"] = [];
    const importedSlugToAgentId = new Map<string, string>();
    const existingSlugToAgentId = new Map<string, string>();
    const existingAgents = await agents.list(targetProject.id);
    for (const existing of existingAgents) {
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const created = await agents.create(targetProject.id, patch);
