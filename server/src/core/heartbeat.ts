...
import { and, asc, desc, eq, gt, inArray, sql } from "@gitmesh/data";
...
const DEFERRED_WAKE_CONTEXT_KEY = "_gitmeshAgentsWakeContext";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__gitmesh-agents_repo_only__";
...
interface WakeupOptions {
  source?: "timer" | "assignment" | "on_demand" | "automation" | "webhook";
  triggerDetail?: "manual" | "ping" | "callback" | "system" | string;
  reason?: string | null;
...
export function heartbeatService(db: Db) {
  ...
  async function getTaskSession(
    ...
    return db
      ...
      .where(
        and(
          eq(agentTaskSessions.projectId, projectId),
  ...
  async function executeRun(runId: string) {
    ...
    const policyAction =
      readNonEmptyString(context.policyAction) ??
      readNonEmptyString(context.action) ??
      (issueId ? "issue.execute" : "heartbeat.execute");
    const policyResult = await policyEngine.evaluate({
      ...
      context: {
        ...
        triggerDetail: readNonEmptyString(context.wakeTriggerDetail),
      },
    });

    if (policyResult.effect !== "allow") {
      const blockedByApproval = policyResult.effect === "require_approval";
      const policyMessage = blockedByApproval
        ? `Policy requires approval before action \"${policyAction}\" can run.`
        : `Policy blocked action \"${policyAction}\".`;

      await setRunStatus(run.id, "cancelled", {
        finishedAt: new Date(),
        error: `${policyMessage} ${policyResult.reason}`,
        errorCode: blockedByApproval ? "policy_requires_approval" : "policy_blocked",
        resultJson: {
          policy: {
      ...
      await setWakeupStatus(run.wakeupRequestId, blockedByApproval ? "pending_approval" : "blocked", {
        finishedAt: new Date(),
        error: `${policyMessage} ${policyResult.reason}`,
      });
      ...
      if (cancelledRun) {
        await appendRunEvent(cancelledRun, 1, {
          eventType: "lifecycle",
          stream: "system",
          level: blockedByApproval ? "warn" : "error",
          message: blockedByApproval ? "run halted: policy requires approval" : "run blocked by policy",
          payload: {
            action: policyAction,
            policyId: policyResult.policyId,
            policyName: policyResult.policyName,
      ...
      if (blockedByApproval) {
        ...
        if (updatedAgent) {
          publishLiveEvent({
            projectId: updatedAgent.projectId,
            type: "agent.status",
            payload: {
    ...
    context.gitmeshWorkspace = workspaceContextValue;
    context.gitmeshWorkspaces = resolvedWorkspace.workspaceHints;
    context.gitmeshAgentsWorkspace = workspaceContextValue;
    context.gitmeshAgentsWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    try {
      ...
      if (runningAgent) {
        publishLiveEvent({
          projectId: runningAgent.projectId,
          type: "agent.status",
          payload: {
      ...
      for (const warning of runtimeWorkspaceWarnings) {
        await onLog("stderr", `[gitmesh-agents] ${warning}\n`);
      }
      ...
      const authToken = adapter.supportsLocalAgentJwt
        ? createLocalAgentJwt(agent.id, agent.projectId, agent.adapterType, run.id)
        : null;
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected GITMESH_API_KEY",
        );
      }
      ...
      if (finalizedRun) {
        ...
        if (taskKey) {
          if (adapterResult.clearSession || (!nextSessionState.params && !nextSessionState.displayId)) {
            await clearTaskSessions(agent.projectId, agent.id, {
              taskKey,
              adapterType: agent.adapterType,
            });
          } else {
  ...
  async function releaseIssueExecutionAndPromote(run: typeof heartbeatRuns.$inferSelect) {
    const promotedRun = await db.transaction(async (tx) => {
      await tx.execute(
        sql`select id from issues where project_id = ${run.projectId} and execution_run_id = ${run.id} for update`,
      );
      ...
      const issue = await tx
        ...
        .from(issues)
        .where(and(eq(issues.projectId, run.projectId), eq(issues.executionRunId, run.id)))
        .then((rows) => rows[0] ?? null);
      ...
      while (true) {
        const deferred = await tx
          ...
          .where(
            and(
              eq(agentWakeupRequests.projectId, issue.projectId),
        ...
        if (
          !deferredAgent ||
          deferredAgent.projectId !== issue.projectId ||
        ...
        const newRun = await tx
          ...
          .values({
            projectId: deferredAgent.projectId,
    ...
    publishLiveEvent({
      projectId: promotedRun.projectId,
  ...
  async function enqueueWakeup(agentId: string, opts: WakeupOptions = {}) {
    ...
    if (issueId && !bypassIssueExecutionLock) {
      ...
      const outcome = await db.transaction(async (tx) => {
        await tx.execute(
          sql`select id from issues where id = ${issueId} and project_id = ${agent.projectId} for update`,
        );
        ...
        if (activeExecutionRun) {
          ...
          const existingDeferred = await tx
            ...
            .where(
              and(
                eq(agentWakeupRequests.projectId, agent.projectId),
  ...
  return {
    list: (projectId: string, agentId?: string, limit?: number) => {
      const query = db
        .select()
        .from(heartbeatRuns)
        .where(
          agentId
            ? and(eq(heartbeatRuns.projectId, projectId), eq(heartbeatRuns.agentId, agentId))
    ...
    getRuntimeState: async (agentId: string) => {
      ...
      const latestTaskSession = await db
        .select()
        .from(agentTaskSessions)
        .where(and(eq(agentTaskSessions.projectId, agent.projectId), eq(agentTaskSessions.agentId, agent.id)))
    ...
    listTaskSessions: async (agentId: string) => {
      ...
      return db
        .select()
        .from(agentTaskSessions)
        .where(and(eq(agentTaskSessions.projectId, agent.projectId), eq(agentTaskSessions.agentId, agentId)))
