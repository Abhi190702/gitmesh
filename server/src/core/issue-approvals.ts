import { and, desc, eq, inArray } from "@gitmesh/data";
import type { Db } from "@gitmesh/data";
import { approvals, issueApprovals, issues } from "@gitmesh/data";
...
export function issueApprovalService(db: Db) {
  ...
  async function assertIssueAndApprovalSameProject(issueId: string, approvalId: string) {
    ...
    if (issue.projectId !== approval.projectId) {
      throw unprocessable("Issue and approval must belong to the same project");
    }
  ...
  return {
    ...
    listIssuesForApproval: async (approvalId: string) => {
      ...
      return db
        .select({
          ...
          subprojectId: issues.subprojectId,
    ...
    link: async (issueId: string, approvalId: string, actor?: LinkActor) => {
      const { issue } = await assertIssueAndApprovalSameProject(issueId, approvalId);
    ...
    unlink: async (issueId: string, approvalId: string) => {
      await assertIssueAndApprovalSameProject(issueId, approvalId);
      await db
    ...
    linkManyForApproval: async (approvalId: string, issueIds: string[], actor?: LinkActor) => {
      ...
      for (const row of rows) {
        if (row.projectId !== approval.projectId) {
          throw unprocessable("Issue and approval must belong to the same project");
        }
      }
