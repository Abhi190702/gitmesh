...
function jwtConfig() {
  const secret = process.env.GITMESH_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.GITMESH_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.GITMESH_AGENT_JWT_ISSUER ?? "gitmesh-agents",
    audience: process.env.GITMESH_AGENT_JWT_AUDIENCE ?? "gitmesh-agents-api",
  };
}
...
export function createLocalAgentJwt(agentId: string, projectId: string, adapterType: string, runId: string) {
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  const projectId = typeof claims.project_id === "string" ? claims.project_id : null;
  ...
  if (!sub || !projectId || !adapterType || !runId || !iat || !exp) return null;
