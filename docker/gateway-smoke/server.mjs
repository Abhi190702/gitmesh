...
const webhookPath = process.env.GATEWAY_SMOKE_PATH?.trim() || "/webhook";
const expectedAuthHeader = process.env.GATEWAY_SMOKE_AUTH?.trim() || "";
...
server.listen(port, "0.0.0.0", () => {
  ...
  console.log(`[gateway-smoke] listening on :${port} path=${webhookPath}`);
});
