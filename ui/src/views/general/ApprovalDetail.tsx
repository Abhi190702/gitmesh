...
import { typeLabel, typeIcon, defaultTypeIcon, ApprovalPayloadRenderer } from "../../features/ApprovalPayload";
...
import type { ApprovalComment } from "@gitmesh/core";
      ...
      <DialogContent className="max-w-sm">
        ...
        <DialogFooter className="mt-3">
          <Button
            ...
            className={variant === "default" ? "bg-green-700 hover:bg-green-600 text-white" : undefined}
...
export function ApprovalDetail() {
  ...
  const resolvedProjectId = approval?.projectId ?? selectedProjectId;
  ...
  useEffect(() => {
    if (!approval?.projectId || approval.projectId === selectedProjectId) return;
    setSelectedProjectId(approval.projectId, { source: "route_sync" });
  }, [approval?.projectId, selectedProjectId, setSelectedProjectId]);
  ...
  const refresh = () => {
    ...
    if (approval?.projectId) {
      queryClient.invalidateQueries({ queryKey: queryKeys.approvals.list(approval.projectId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.approvals.list(approval.projectId, "pending") });
      queryClient.invalidateQueries({ queryKey: queryKeys.agents.list(approval.projectId) });
    }
  };
  ...
  const addCommentMutation = useMutation({
    ...
    onSuccess: () => { setCommentBody(""); setError(null); refresh(); },
    onError: (err) => setError(err instanceof Error ? err.message : "Comment failed"),
  });

  const deleteAgentMutation = useMutation({
    mutationFn: (agentId: string) => agentsApi.remove(agentId),
    onSuccess: () => { setError(null); refresh(); navigate("/approvals"); },
  ...
  const resolvedCta = primaryLinkedIssue
    ? { label: (linkedIssues?.length ?? 0) > 1 ? "Review linked issues" : "Review linked issue", to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}` }
    : linkedAgentId
    ? { label: "Open enabled agent", to: `/agents/${linkedAgentId}` }
    : { label: "Back to approvals", to: "/approvals" };

  const requesterLabel = approval.requestedByAgentId
    ? agentNameMap.get(approval.requestedByAgentId) ?? approval.requestedByAgentId.slice(0, 8)
    : approval.requestedByUserId
    ? approval.requestedByUserId === "local-board" ? "Maintainer" : approval.requestedByUserId
    : "Unknown";
    ...
    <div className="space-y-6 max-w-3xl">
      {showApprovedBanner && (
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            ...
            <Button size="sm" variant="outline" className="border-green-400 dark:border-green-600/50 text-green-800 dark:text-green-100 hover:bg-green-100 dark:hover:bg-green-900/30" onClick={() => navigate(resolvedCta.to)}>
              {resolvedCta.label}
            </Button>
          </div>
        </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground font-mono">
                ...
                <span>·</span>
                <span title={new Date(approval.createdAt).toLocaleString()}>{timeAgo(approval.createdAt)}</span>
              </div>
            </div>
          </div>
        ...
        <div className="text-sm space-y-1">
          <div className="flex items-center gap-2">
            ...
            <Identity name={requesterLabel} size="sm" />
          </div>
          ...
          <div className="pt-2 border-t border-border/60">
            ...
            <div className="space-y-1.5">
              {linkedIssues.map((issue) => (
                <Link key={issue.id} to={`/issues/${issue.identifier ?? issue.id}`} className="block text-xs rounded border border-border/70 px-2 py-1.5 hover:bg-accent/20">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <NoteDialog
            trigger={<Button size="sm" className="bg-green-700 hover:bg-green-600 text-white" disabled={approveMutation.isPending || !isActionable}>Approve</Button>}
            title="Approve this request"
            description="Optionally add a note explaining your decision."
            onSubmit={(note) => approveMutation.mutate(note || undefined)}
            isPending={approveMutation.isPending}
          />

          <NoteDialog
            trigger={<Button variant="destructive" size="sm" disabled={rejectMutation.isPending || !isActionable}>Reject</Button>}
            title="Reject this request"
            description="Optionally add a note explaining why this was rejected."
            onSubmit={(note) => rejectMutation.mutate(note || undefined)}
          ...
          {approval.status === "pending" && (
            <NoteDialog
              trigger={<Button size="sm" variant="outline" disabled={revisionMutation.isPending}>Request revision</Button>}
              title="Request a revision"
              description="Add notes describing what changes are needed."
              onSubmit={(note) => revisionMutation.mutate(note || undefined)}
          ...
          {approval.status === "revision_requested" && (
            <Button size="sm" variant="outline" onClick={() => resubmitMutation.mutate()} disabled={resubmitMutation.isPending}>
              Resubmit
            </Button>
          )}

          {approval.status === "rejected" && approval.type === "enable_agent" && linkedAgentId && (
            <Button
              size="sm"
              variant="outline"
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
              ...
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  <Link to={`/agents/${comment.authorAgentId}`} className="hover:underline">
                    <Identity name={agentNameMap.get(comment.authorAgentId) ?? comment.authorAgentId.slice(0, 8)} size="sm" />
                  </Link>
                ) : (
                  <Identity name="Maintainer" size="sm" />
                )}
        ...
        <div className="flex justify-end">
          <Button size="sm" onClick={() => addCommentMutation.mutate()} disabled={!commentBody.trim() || addCommentMutation.isPending}>
