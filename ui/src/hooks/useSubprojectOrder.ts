...
function buildOrderIds(projects: Subproject[], orderedIds: string[]) {
  return sortProjectsByStoredOrder(projects, orderedIds).map((project) => project.id);
}

export function useSubprojectOrder({ projects, projectId, userId }: UseProjectOrderParams) {
  const storageKey = useMemo(() => {
    if (!projectId) return null;
    return getProjectOrderStorageKey(projectId, userId);
  }, [projectId, userId]);
  ...
  useEffect(() => {
    ...
    window.addEventListener(SUBPROJECT_ORDER_UPDATED_EVENT, onCustomEvent);
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener(SUBPROJECT_ORDER_UPDATED_EVENT, onCustomEvent);
    };
  }, [projects, storageKey]);
