import type { Approval, Issue, IssueAttachment, IssueComment, IssueLabel } from "@gitmesh/core";
...
export const issuesApi = {
  list: (
    ...
    return api.get<Issue[]>(`/projects/${projectId}/issues${qs ? `?${qs}` : ""}`);
  },
  listLabels: (projectId: string) => api.get<IssueLabel[]>(`/projects/${projectId}/labels`),
  createLabel: (projectId: string, data: { name: string; color: string }) =>
    api.post<IssueLabel>(`/projects/${projectId}/labels`, data),
  ...
  uploadAttachment: (
    ...
    return api.postForm<IssueAttachment>(`/projects/${projectId}/issues/${issueId}/attachments`, form);
  },
