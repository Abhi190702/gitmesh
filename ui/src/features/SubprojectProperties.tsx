...
interface SubprojectPropertiesProps {
  project: Subproject;
...
export function SubprojectProperties({ project, onUpdate }: SubprojectPropertiesProps) {
  ...
  const linkedMilestones = (project.goals ?? []).length > 0
    ? (project.goals ?? [])
    : linkedGoalIds.map((id: string) => ({
        id,
  ...
  const invalidateProject = () => {
    queryClient.invalidateQueries({ queryKey: queryKeys.subprojects.detail(project.id) });
  ...
  const createWorkspace = useMutation({
    mutationFn: (data: Record<string, unknown>) => subprojectsApi.createWorkspace(project.id, data),
    onSuccess: () => {
      setWorkspaceCwd("");
      setWorkspaceRepoUrl("");
  ...
  const removeWorkspace = useMutation({
    mutationFn: (workspaceId: string) => subprojectsApi.removeWorkspace(project.id, workspaceId),
    onSuccess: invalidateProject,
  });
  const updateWorkspace = useMutation({
    mutationFn: ({ workspaceId, data }: { workspaceId: string; data: Record<string, unknown> }) =>
      subprojectsApi.updateWorkspace(project.id, workspaceId, data),
    onSuccess: invalidateProject,
  });

  const removeMilestone = (goalId: string) => {
    if (!onUpdate) return;
  ...
  const clearLocalWorkspace = (workspace: SubprojectWorkspace) => {
  ...
  const clearRepoWorkspace = (workspace: SubprojectWorkspace) => {
                    ...
                    >
                      ...
                      {onUpdate && (
                        <button
                          ...
                          type="button"
                          onClick={() => removeMilestone(goal.id)}
