...
import { useLiveEventSocket } from "../hooks/useLiveEventSocket";
...
/**
 * GitMesh-specific tone palette for the live run feed. Centralized
 * so the renderer doesn't ad-hoc string-match tone names.
 */
const FEED_TONE_CLASS: Record<FeedTone, string> = {
  info: "text-foreground/80",
  warn: "text-amber-600 dark:text-amber-300",
  error: "text-red-600 dark:text-red-300",
  assistant: "text-emerald-700 dark:text-emerald-200",
...
export function LiveRunWidget({ issueId, projectId }: LiveRunWidgetProps) {
  ...
  const handleLiveEvent = useCallback(
    ...
    [activeRunIds, projectId, runById],
  );

  useLiveEventSocket({
    projectId: projectId ?? null,
    enabled: activeRunIds.size > 0,
  ...
  return (
    <div className="rounded-lg border border-border bg-card overflow-hidden">
      {runs.length > 0 ? (
        runs.map((run) => (
          <div key={run.id} className="px-3 py-2 border-b border-border">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <Link
                ...
                className="inline-flex items-center rounded-md border border-border bg-muted px-2 py-1 font-mono text-muted-foreground hover:text-foreground hover:bg-accent transition-colors"
              >
                {run.id.slice(0, 8)}
              </Link>
      ...
      <div ref={bodyRef} className="max-h-[220px] overflow-y-auto p-2 font-mono text-[11px] space-y-1">
        ...
        {recent.map((item, index) => (
          ...
          >
            ...
            <div className={cn("min-w-0", FEED_TONE_CLASS[item.tone])}>
