...
import type { Issue, LiveEvent } from "@gitmesh/core";
...
export function ActiveAgentsPanel({ projectId }: ActiveAgentsPanelProps) {
  ...
  const { data: liveRuns } = useQuery({
    queryKey: [...queryKeys.liveRuns(projectId), "dashboard"],
    queryFn: () => heartbeatsApi.liveRunsForProject(projectId, MIN_DASHBOARD_RUNS),
  });
  ...
  return (
    <div>
      <h3 className="mb-3 font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-text-tertiary">
        Live & recent runs
      </h3>
      {runs.length === 0 ? (
...
function AgentRunCard({
  ...
  return (
    <div className={cn(
      "flex min-h-[160px] flex-col overflow-hidden rounded-lg border border-border bg-card",
    )}>
      {/* Header */}
