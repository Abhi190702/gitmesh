...
export function normalizeProjectPrefix(prefix: string): string {
...
export function extractProjectPrefixFromPath(pathname: string): string | null {
  ...
  return normalizeProjectPrefix(segments[0]!);
}

export function applyProjectPrefix(path: string, projectPrefix: string | null | undefined): string {
  const { pathname, search, hash } = splitPath(path);
  if (!pathname.startsWith("/")) return path;
  if (isGlobalPath(pathname)) return path;
  if (!projectPrefix) return path;

  const prefix = normalizeProjectPrefix(projectPrefix);
  const activePrefix = extractProjectPrefixFromPath(pathname);
