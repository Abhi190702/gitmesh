...
type GitmeshSprite = (typeof GITMESH_SPRITES)[number];
...
function spriteSize(sprite: GitmeshSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = GITMESH_SPRITES[(baseCol + baseRow) % GITMESH_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = GITMESH_SPRITES[Math.floor(Math.random() * GITMESH_SPRITES.length)]!;
