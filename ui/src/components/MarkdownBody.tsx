...
import { parseProjectMentionHref } from "@gitmesh/core";
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`gitmesh-agents-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="gitmesh-agents-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("gitmesh-agents-mermaid-status", error && "gitmesh-agents-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="gitmesh-agents-mermaid-source">
            ...
            if (parsed) {
              ...
              return (
                <a
                  ...
                  className="gitmesh-agents-project-mention-chip"
