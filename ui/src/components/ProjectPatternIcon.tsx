...
interface ProjectPatternIconProps {
  projectName: string;
...
function makeProjectPatternDataUrl(seed: string, brandColor?: string | null, logicalSize = 22, cellSize = 2): string {
...
export function ProjectPatternIcon({ projectName, brandColor, className }: ProjectPatternIconProps) {
  const initial = projectName.trim().charAt(0).toUpperCase() || "?";
  const patternDataUrl = useMemo(
    () => makeProjectPatternDataUrl(projectName.trim().toLowerCase(), brandColor),
    [projectName, brandColor],
  );
